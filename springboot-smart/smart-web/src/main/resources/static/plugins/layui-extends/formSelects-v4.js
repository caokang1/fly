...
(function (layui, window, factory) {
	...
	var v = '4.0.0.0910',
		...
		this.config = {
			...
			init: null, //初始化的选择值,
			on: null, //select值发生变化
			isShowKey: false, //smart，如果key-value长度太大，选中时在input框很难看，需要调整为只显示key
			opened: null,
			closed: null,
	...
	Common.prototype.createDD = function (id, item, clz) {
		...
		return '<dd lay-value="' + item.value + '" class="' + (item.disabled ? DISABLED : '') + ' ' + (clz ? clz : '') + '" ' + attr + '>\n\t\t\t\t\t<div title="'/*smart给超长的内容显示title*/+name+'" class="xm-unselect xm-form-checkbox ' + (item.disabled ? DISABLED : '') + '"  style="margin-left: ' + (pid.length - 1) * 30 + 'px">\n\t\t\t\t\t\t<i class="' + CHECKBOX_YES + '"></i>\n\t\t\t\t\t\t<span name="' + name + '">' + template + '</span>\n\t\t\t\t\t</div>\n\t\t\t\t</dd>';
	};
	...
	Common.prototype.renderSelect = function (id, tips, select) {
		...
		if (data[id].config.btns.length) {
			setTimeout(function () {
				...
				dl.find('.' + CZ_GROUP).css('max-width', dl.prev().width() /*- 54*/ + 'px');//smart,这里控制按钮的父div的总宽度，原来的太窄
			}, 10);
			arr.push(['<dd lay-value="" class="' + FORM_SELECT_TIPS + '" style="background-color: #FFF!important;">', /*this.renderBtns(id, null, '30px'),smart,这里控制按钮的间距，原来的太宽*/this.renderBtns(id, null, '12px'), '</dd>', '<dd lay-value="" class="' + FORM_SELECT_TIPS + ' ' + FORM_DL_INPUT + '" style="background-color: #FFF!important;">', '<i class="xm-iconfont icon-sousuo"></i>', '<input type="text" class="' + FORM_INPUT + ' ' + INPUT + '" placeholder="\u8BF7\u641C\u7D22"/>', '</dd>'].join(''));
		} else {
	...
	Common.prototype.addLabel = function (id, div, val) {
		...
		//smart,如果key-value长度太大，选中时在input框很难看，需要调整为只显示key
		if(!fs.config.isShowKey){
			_ref = [$('<span ' + tips + ' value="' + val.value + '"><font ' + tips + '>' + val.name + '</font></span>'), $('<i ' + tips + ' class="xm-iconfont icon-close"></i>')];
		}
		else{
			_ref = [$('<span ' + tips + ' value="' + val.value + '"><font ' + tips + '>' + val.value + '</font></span>'), $('<i ' + tips + ' class="xm-iconfont icon-close"></i>')];
		}
...
//				});
	...
	Select4.prototype.render = function (id, options) {
		...
		if (!!window.ActiveXObject || "ActiveXObject" in window){
			//smart
			//如果是IE，不支持_defineProperty
			config = options ? (_ref2 = {
					init: options.init,
					...
					isShowKey: options.isShowKey,//smart,如果key-value长度太大，选中时在input框很难看，需要调整为只显示key
					on: options.on
				},  _ref2) : {};
